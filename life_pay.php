...
if(isset($_SESSION['url'])){
  //print_r($_SESSION);
  if(isset($_GET['c']) && isset($_GET['st']) && isset($_GET['li'])){
    $c = $_GET['c'];//判斷是不是在活動
    $st = $_GET['st'];
    $li = $_GET['li'];
    header(sprintf("Location: person_main.php?c=$c&st=$st&li=$li"));
  }else if(isset($_GET['a'])){
    $mkey = $_GET['a'];
    header("Location: person_main.php?a=$mkey");exit;
  }else{
    header(sprintf("Location: person_main.php"));exit;
  }
}
        ...
        if ($_POST['sum'] != "" && $_POST['sum'] == $_POST['cksum']){
          ...
          {//echo "ok";exit;
            ...
            if($totalRows_row_str != 0){
              ...
              header(sprintf("Location: person_main.php?c=$c&st=$st&li=$li"));
            }else{header(sprintf("Location: person_main.php"));}
          }else {$a = "會員資料不對!!!";}
        }else {$a = "驗證碼錯誤!!!";}
      //}else {$a = "二級密碼不可空白!!!";}
    }else {$a="密碼不可空白!!!";}
