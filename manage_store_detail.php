...
<form id="form1" action="manage_store_detail.php" method="get">
...
<div class="search_table table-responsive " align="center" style="overflow-y: visible;white-space: nowrap;padding-top: 10px">
    ...
    <?php if($row_num != 0){
      do{?>
        <tr class="search_tr"  style="background-color: #fff">
          ...
          <td><?php echo $row_str['usage_fee'];?></td>
  ...
  <div style="width: 88%" align="center"><span>銷售紀錄</span></div>
    ...
    <tr style="background-color: #4ab3a6;color: #fff ">
      ...
      <th>抵用券</th>
      <th>現金/信用卡</th>
      <th style="border-radius: 0px 10px 0px 0px">回饋金額</th>
		</tr>
    <?php 
    ...
    if($row_num != 0){
      do{ 
	  	//銷售金額
        $tatal_sell = $row_str['total'];
        $sell_sum = $sell_sum + $tatal_sell;
        //消費積分總額
        $g = $row_str['g'];
        $g_sum = $g_sum + $g;
        //串串積分總額
